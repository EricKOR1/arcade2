...
const TRACKS = {
  meadow: {
    ...
    bg: '#16261C', grass: '#1E3427', road: '#707479',
  ...
  figure8: {
    ...
    scale: 6, width: 1440, laps: 3, speed: 18.8,
    bg: '#0B1020', grass: '#121A2E', road: '#6D7079',
  ...
  canyon: {
    ...
    bg: '#3A2417', grass: '#5A3A22', road: '#928C91',
  ...
  lake: {
    ...
    bg: '#0F1E2E', grass: '#326248', road: '#9DA0A4',
  ...
  neon: {
    ...
    bg: '#0A0D1A', grass: '#1A1F2E', road: '#6B6F79',
  ...
  serpent: {
    ...
    bg: '#2B1A14', grass: '#6E482E', road: '#9C9CA2',
  ...
  skybridge: {
    ...
    tag: '★★★ 중간 · 가장 긴 코스(약 2분 30초) · 점프 1', difficulty: 3, jumps: [0.62],
...
class Track {
  constructor(def) {
    ...
    // 카트 크기 — 도로 폭의 6.75% (도로가 카트보다 약 15배 넓게 느껴집니다)
    this.carLen = def.width * 0.0675;
    this.carScale = this.carLen / 40;
    this.pickR = def.width * 0.07;        // 아이템 상자 획득 반경
  ...
  buildScenery() {
    ...
    for (let k = 0; k < oCount; k++) {
      const i = (60 + k * oGap + Math.floor(orand() * oGap * 0.5)) % this.n;
      if (i < 40 || i > this.n - 20) continue;
      if (this.itemSpots.some(sp => Math.abs(sp.i - i) < 6)) continue;
      const kind = okinds[Math.floor(orand() * okinds.length)];
      const off = (orand() * 1.4 - 0.7);                     // 도로 안쪽 어딘가
      const [tx, ty] = this.tangent[i], nx = -ty, ny = tx;
      this.obstacles.push({ kind: kind, i: i,
        x: this.center[i][0] + nx * this.halfW * off, y: this.center[i][1] + ny * this.halfW * off,
        hitUntil: 0, r: orand() });
    }
  }
...
const ITEMS = {
  ...
  oil:     { icon: '🛢', name: '기름',     hint: '뒤에 뿌립니다 — 밟으면 미끄러짐' },
  bomb:    { icon: '💣', name: '폭탄',     hint: '뒤에 놓습니다 — 가까이 오면 폭발' },
  boost:   { icon: '⚡', name: '부스터',   hint: '2.2초간 가속' },
  star:    { icon: '⭐', name: '스타',     hint: '6초간 무적 + 가속' },
...
const OBSTACLE = {
  cone:   { name: '콘',       r: 0.62, effect: 'bump',  color: '#FF8A56' },
  barrel: { name: '드럼통',   r: 0.80, effect: 'spin',  color: '#4CC9F0' },
  rock:   { name: '바위',     r: 0.85, effect: 'spin',  color: '#8A919E' },
  puddle: { name: '물웅덩이', r: 0.90, effect: 'slide', color: '#3B82F6' },
  ice:    { name: '얼음',     r: 0.95, effect: 'slide', color: '#BFE9FF' }
};
...
class KartGame {
  ...
  useItem() {
    ...
    if (it === 'turtle' && !this.nextBehind()) {
      this.showToast('뒤에 아무도 없다', '#9AA3B2');
      return;
    }
    this.items.shift();
    ...
    if (it === 'boost2') {
      ...
      this.showToast({ banana: '바나나를 놓았다', oil: '기름을 뿌렸다', bomb: '폭탄을 놓았다' }[it],
                     { banana: '#FFD166', oil: '#9AA3B2', bomb: '#FF5C7A' }[it]);
  ...
  nextBehind() {
    ...
    Object.keys(this.peers).forEach(id => {
      ...
      if (pr < this.progress && pr > bp) { bp = pr; best = id; }
    });
    return best;
  }
  ...
  guarded() {
    const now = this.clock();
    return now < this.shieldUntil || now < this.starUntil;
  }


  hitByMissile(byId, kind) {
    if (this.finished) return;
    if (this.guarded()) { this.showToast('방어막이 막았다', '#06D6A0'); return; }
    this.stunUntil = this.clock() + 1500;
    this.speed = 0;
    this.addFx('hit');
  ...
  hitByBolt(byId) {
    if (this.finished) return;
    if (this.guarded()) { this.showToast('방어막이 막았다', '#06D6A0'); return; }
    this.slowUntil = this.clock() + 1800;
    this.showHit('🌩', byId, '의 번개에 맞았다', '#B15DFF');
  }
  ...
  spinOut() {
    if (this.clock() < this.spinUntil || this.finished || this.guarded()) return;
    this.spinUntil = this.clock() + 1300;
    this.speed *= 0.2;
    if (window.Sound) Sound.crash();
    this.showToast('미끄러졌다!', '#FFD166');
  }

  // 미끄러짐 — 1초 동안 핸들이 안 듣고 속도는 유지
  slide() {
    const now = this.clock();
  ...
  showToast(text, color) { this.toast = { text, color: color || '#fff', until: this.clock() + 1500 }; }

  // 누구에게 당했는지 — 화면 가운데에 크게 (2초)
  nameOf(id) {
    if (!id) return '';
    if (this.peers[id] && this.peers[id].name) return this.peers[id].name;
    if (this.opts.nameOf) return this.opts.nameOf(id) || '';
    return '';
  }
  ...
  addFx(type) { this.fx.push({ type, until: this.clock() + 500 }); }
  ...
  spectate(followId) {
    ...
    this.countdown = 0;
    this.items = [];
    const p = this.peers[followId];
    if (p) { this.x = p.x; this.y = p.y; this.angle = p.angle; this.camX = p.x; this.camY = p.y; }
  }
  ...
  tick(now) {
    ...
    // 관전 모드 — 물리는 돌리지 않고, 따라가는 학생의 위치로 카메라만 옮깁니다
    if (this.spectator) {
  ...
  updateCar(now, f) {
    ...
    if (this.finished) {
      ...
      // 결승 순서(finishRank)를 진행도에 얹습니다.
      this.progress = this.track.laps * n + (this.finishRank ? (1000 - this.finishRank) : 0);
    } else {
    ...
    if (!this.airborne) Object.keys(this.hazards).forEach(hid => {
      ...
      if (Math.hypot(h.x - this.x, h.y - this.y) < reach) {
        ...
        const g = this.guarded();
        if (kind === 'oil') this.slide();
        else if (kind === 'bomb') this.explode(h.x, h.y);
        else this.spinOut();
        if (!g && h.by && h.by !== this.myId) {
          if (kind === 'oil') this.showHit('🛢', h.by, '이(가) 뿌린 기름을 밟았다', '#9AA3B2');
          else if (kind === 'bomb') this.showHit('💣', h.by, '이(가) 놓은 폭탄이 터졌다', '#FF5C7A');
          else this.showHit('🍌', h.by, '이(가) 놓은 바나나를 밟았다', '#FFD166');
        }
      }
    });

    // 장애물 — 트랙에 고정된 콘 · 드럼통 · 바위 · 물웅덩이 · 얼음
    this.track.obstacles.forEach(o => {
      if (now < o.hitUntil) return;
      const size = OBSTACLE[o.kind] || OBSTACLE.cone;
      const odx = this.x - o.x, ody = this.y - o.y, odd = Math.hypot(odx, ody) || 1;
      if (odd < this.track.carLen * size.r) {
        o.hitUntil = now + 2500;
        // 어떤 장애물이든 일단 밖으로 밀어냅니다 — 제자리에서 계속 부딪히는 일이 없도록
        const out = this.track.carLen * size.r - odd + this.track.carLen * 0.35;
        this.x += odx / odd * out; this.y += ody / odd * out;
        if (this.guarded()) { this.showToast('방어막이 막았다', '#06D6A0'); return; }
        if (size.effect === 'slide') this.slide();
    ...
    // 카트끼리 충돌 — 겹치면 밀어내되, 밀리는 양에 상한을 둬서 서로 맞물려 멈추는 일이 없게 합니다.
    // 정면으로 겹치면 옆으로 비껴나고, 뒤에서 들이받은 쪽만 살짝 감속합니다.
    if (!this.finished && now > this.spinUntil && (!this.startedAt || now - this.startedAt > 2500)) {
      const minD = this.track.carLen * 0.85;
      const fx = Math.cos(this.angle), fy = Math.sin(this.angle);
      let px = 0, py = 0, bumpedAhead = false, cx = 0, cy = 0, cnt = 0;
      Object.keys(this.peers).forEach(id => {
        const p = this.peers[id];
        if (p.finished) return;
        const dx = this.x - p.x, dy = this.y - p.y, dd = Math.hypot(dx, dy);
        if (dd > 0.001 && dd < minD) {
          const overlap = minD - dd;
          // 밀려나는 방향: 상대 → 나. 정면(진행 방향과 거의 평행)이면 옆으로 살짝 틀어 줍니다
          let ux = dx / dd, uy = dy / dd;
          const along = Math.abs(ux * fx + uy * fy);
          if (along > 0.85) { const sgn = ((ux * -fy + uy * fx) >= 0) ? 1 : -1; ux = ux * 0.5 + (-fy) * sgn * 0.6; uy = uy * 0.5 + fx * sgn * 0.6; }
          px += ux * overlap * 0.5; py += uy * overlap * 0.5;
          if ((-dx * fx - dy * fy) > 0) bumpedAhead = true;      // 상대가 내 앞 = 내가 들이받음
          cx += (this.x + p.x) / 2; cy += (this.y + p.y) / 2; cnt++;
        }
      });
      if (cnt) {
        // 한 프레임에 밀리는 양은 내 전진량의 90%를 넘지 않습니다 (뚫고 가지도, 서로 맞물려 멈추지도 않게)
        const mag = Math.hypot(px, py), cap = Math.max(this.track.carLen * 0.06, this.speed * f * 0.9);
        if (mag > cap) { px *= cap / mag; py *= cap / mag; }
        this.x += px; this.y += py;
        if (bumpedAhead && now - (this.lastBump || 0) > 400) {
          this.lastBump = now;
  ...
  nearBoostPad() {
    const t = this.track, reach = t.stepLen * 40;
  ...
  // 투영 결과 객체를 돌려 씁니다. 프레임당 2,700번 호출되는데 매번 새로 만들면
  // 초당 16만 개가 버려져 가비지 수집이 돌 때마다 화면이 순간 멈춥니다.
  // 한 프레임 동안만 유효 — 값을 오래 들고 있어야 하면 x·y 를 따로 복사해 두세요.
  project(cam, wx, wy, h) {
    const dx = wx - cam.x, dy = wy - cam.y;
    const lz =  dx * cam.cos + dy * cam.sin;
    if (lz < cam.near) return null;
    const lx = -dx * cam.sin + dy * cam.cos;
    const pool = this._prjPool || (this._prjPool = []);
    let o = pool[this._prjN];
    if (!o) { o = { x: 0, y: 0, s: 0, z: 0 }; pool[this._prjN] = o; }
    this._prjN = (this._prjN + 1) % 4096;          // 한 프레임에 쓰는 개수보다 넉넉하게
    o.x = cam.W / 2 + cam.f * lx / lz;
    o.y = cam.horizonY + cam.f * (cam.absH - (h || 0)) / lz;
    o.s = cam.f / lz; o.z = lz;
    return o;
  }
  ...
  drawSky(ctx, W, H, cam) {
    ...
    if (sk.stars > 0) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const tw = 0.4 + Math.sin(this.clock() / 700 + i) * 0.35;
  ...
  drawRoad(ctx, W, H, cam, now) {
    ...
    const Q = (this.q == null ? 2 : this.q);
    // 시야: 실제 거리로 맞춥니다. 구간 수 상한이 낮으면 구간이 촘촘한 코스(스카이 브릿지 등)만
    // 거리가 2/3 로 줄어 도로가 늦게 떴습니다 — 상한을 거리에 맞춰 올렸습니다.
    const K = Math.min([340, 440, 620][Q], Math.ceil([5200, 7000, 9500][Q] / t.stepLen));
    ...
    // 안전망: 지평선 아래를 지면색으로 덮되, 지평선 가까이는 안개색으로 자연스럽게 이어집니다.
    {
      ...
      // 지평선 근처 띠만 그라데이션(비싼 칠), 그 아래는 단색 — 어차피 도로·갓길이 덮습니다
      const band = Math.min(H - y0, H * 0.22);
      ctx.fillStyle = grassDark;
      if (H - y0 > band) ctx.fillRect(0, y0 + band, W, H - y0 - band);
      const gg = ctx.createLinearGradient(0, y0, 0, y0 + band);
    ...
    runs.forEach((run, ri) => {
      ...
      // 갓길은 지평선 아래 바탕과 같은 색이라 대부분 겹칩니다 — 도로 폭의 1.35배까지만 칠해
      // 언덕에서 도로가 지평선 위로 뜰 때만 받쳐 줍니다 (화면 덧칠 3.1배 → 1.6배)
      stripe(run, 1.35, -1.35, grassDark, toBottom);
  ...
  drawArch(ctx, cam, i, label, isStart) {
    const t = this.track;
    const c = t.center[i], e = t.elev[i];
    if (Math.hypot(c[0] - this.x, c[1] - this.y) > t.stepLen * 180) return;
    ...
    if (isStart) {
      ...
      [1, -1].forEach(side => {
        ...
        if (!f0 || !f1) return;
        const flap = Math.sin(this.clock() / 260 + side) * 3;
  ...
  drawDeco(ctx, cam, o, now, far) {
    ...
    if (far) {
      ...
      if (!top) return;
      ctx.fillStyle = f[2] || this.shade(t.def.grass, -0.35);
      const w = Math.max(1.5, u * f[1] * s);
      if (o.kind === 'tree' || o.kind === 'pine' || o.kind === 'palm' || o.kind === 'rock' || o.kind === 'balloon') {
        ctx.beginPath(); ctx.ellipse(top.x, (top.y + base.y) / 2, w / 2, (base.y - top.y) / 2, 0, 0, Math.PI * 2); ctx.fill();
      } else {
  ...
  drawKart3D(ctx, cam, x, y, e, angle, look, name, isMe, spinning, shield, boosting, now, allowFull, lod) {
    ...
    // 헤드라이트 — 밤 테마에서는 앞에 빛 웅덩이
    if (full && t.def.sky && t.def.sky.stars >= 0.5) {
      const hl = fwd(L * 0.62, 0);
      const hp = this.project(cam, hl.x, hl.y, e + Hh * 0.5);
      if (hp) {
        const g = ctx.createRadialGradient(hp.x, hp.y, 0, hp.x, hp.y, L * 0.9 * hp.s);
        g.addColorStop(0, 'rgba(255,240,200,0.35)'); g.addColorStop(1, 'rgba(255,240,200,0)');
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(hp.x, hp.y, L * 0.9 * hp.s, 0, Math.PI * 2); ctx.fill();
      }
    }
  ...
  drawHazard3D(ctx, cam, x, y, e, kind, now) {
    const t = this.track;
    ...
    if (!g || !u) return;
    if (kind === 'oil') {
      // 기름 — 도로에 번진 검은 웅덩이 + 무지개 광택
      ctx.fillStyle = 'rgba(10,12,18,0.85)';
      ctx.beginPath(); ctx.ellipse(g.x, g.y, r*g.s*1.9, r*g.s*0.8, 0.3, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = 'rgba(177,93,255,0.25)';
      ctx.beginPath(); ctx.ellipse(g.x - r*g.s*0.4, g.y - r*g.s*0.15, r*g.s*0.8, r*g.s*0.3, 0.3, 0, Math.PI*2); ctx.fill();
      return;
    }
    if (kind === 'bomb') {
      // 폭탄 — 검은 공 + 깜빡이는 심지
      ...
      const rr = r*u.s*0.9;
      ctx.fillStyle = '#1A1D24';
      ctx.beginPath(); ctx.arc(u.x, u.y, rr, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,0.25)';
      ctx.beginPath(); ctx.arc(u.x - rr*0.3, u.y - rr*0.35, rr*0.3, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = (Math.floor(now / 160) % 2) ? '#FF5C7A' : '#FFD166';
      ctx.beginPath(); ctx.arc(u.x + rr*0.4, u.y - rr*1.05, Math.max(1.5, rr*0.28), 0, Math.PI*2); ctx.fill();
      return;
    }
    ctx.fillStyle = 'rgba(0,0,0,0.3)';
    ctx.beginPath(); ctx.ellipse(g.x, g.y, r*g.s*1.1, r*g.s*0.5, 0, 0, Math.PI*2); ctx.fill();
  ...
  // 장애물 — 콘 · 드럼통 · 바위 · 물웅덩이 · 얼음
  drawObstacle3D(ctx, cam, o, now) {
    const t = this.track, cl = t.carLen;
    const base = this.project(cam, o.x, o.y, o.e);
    if (!base) return;
    const s = base.s;
    if (o.kind === 'puddle' || o.kind === 'ice') {
      const ice = o.kind === 'ice';
      ctx.fillStyle = ice ? 'rgba(191,233,255,0.55)' : 'rgba(59,130,246,0.45)';
      ctx.beginPath(); ctx.ellipse(base.x, base.y, cl*0.9*s, cl*0.38*s, 0, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,' + (0.25 + Math.sin(now/300 + o.r*6)*0.15).toFixed(2) + ')';
      ctx.beginPath(); ctx.ellipse(base.x - cl*0.3*s, base.y - cl*0.1*s, cl*0.3*s, cl*0.1*s, 0, 0, Math.PI*2); ctx.fill();
      return;
    }
    ctx.fillStyle = 'rgba(0,0,0,0.3)';
    ctx.beginPath(); ctx.ellipse(base.x, base.y, cl*0.45*s, cl*0.2*s, 0, 0, Math.PI*2); ctx.fill();
    if (o.kind === 'cone') {
      const top = this.project(cam, o.x, o.y, o.e + cl*0.55);
      if (!top) return;
      const w = cl*0.36*s;
      ctx.fillStyle = '#FF8A56';
      ctx.beginPath(); ctx.moveTo(base.x - w, base.y); ctx.lineTo(base.x + w, base.y); ctx.lineTo(top.x, top.y); ctx.closePath(); ctx.fill();
      ctx.fillStyle = '#F4F6FA';
      const my = base.y + (top.y - base.y) * 0.45;
      ctx.fillRect(base.x - w*0.55, my, w*1.1, Math.max(1, (base.y - top.y) * 0.12));
      ctx.fillStyle = '#1A1D24'; ctx.fillRect(base.x - w*1.1, base.y - 1, w*2.2, Math.max(1, cl*0.05*s));
    } else if (o.kind === 'barrel') {
      const top = this.project(cam, o.x, o.y, o.e + cl*0.62);
      if (!top) return;
      const w = cl*0.3*s, h = base.y - top.y;
      ctx.fillStyle = '#2B6CB0'; ctx.fillRect(base.x - w, top.y, w*2, h);
      ctx.fillStyle = '#4CC9F0'; ctx.fillRect(base.x - w, top.y + h*0.3, w*2, h*0.14); ctx.fillRect(base.x - w, top.y + h*0.65, w*2, h*0.14);
      ctx.fillStyle = '#5FB3E0'; ctx.beginPath(); ctx.ellipse(base.x, top.y, w, w*0.35, 0, 0, Math.PI*2); ctx.fill();
    } else {                                                  // rock
      const top = this.project(cam, o.x, o.y, o.e + cl*0.5);
      if (!top) return;
      ctx.fillStyle = '#6B7280';
      ctx.beginPath(); ctx.ellipse(base.x, base.y, cl*0.5*s, base.y - top.y, 0, Math.PI, 0); ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,0.15)';
      ctx.beginPath(); ctx.ellipse(base.x - cl*0.15*s, base.y - (base.y - top.y)*0.5, cl*0.16*s, (base.y - top.y)*0.25, 0, 0, Math.PI*2); ctx.fill();
    }
  }
  ...
  drawCanvasOverlay(ctx, W, H, now) {
    ...
    if (this.spectator) {
      ctx.save();
      ctx.font = '700 13px Pretendard, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      const label = '👁 ' + (this.myName || '') + ' 의 화면';
      const tw = ctx.measureText(label).width + 28;
      ctx.fillStyle = 'rgba(8,10,16,0.75)';
      if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(W/2 - tw/2, 12, tw, 30, 15); ctx.fill(); }
      ctx.fillStyle = '#FFD166'; ctx.fillText(label, W/2, 27);
      ctx.restore();
    }

    // 아날로그 속도계 — 왼쪽 아래
    if (this.countdown <= 0 && !this.finished && !this.spectator) {
      const R = Math.min(64, W * 0.17), cx = 16 + R, cy = H - 18;
      const pct = Math.min(1, this.speed / (this.maxSpeed * 1.9));
      const a0 = Math.PI * 1.0, a1 = Math.PI * 2.0;
      ctx.save();
      ctx.fillStyle = 'rgba(8,10,16,0.72)';
      ctx.beginPath(); ctx.arc(cx, cy, R + 8, a0, a1); ctx.lineTo(cx + R + 8, cy + 6); ctx.lineTo(cx - R - 8, cy + 6); ctx.closePath(); ctx.fill();
      ctx.lineCap = 'round';
      ctx.strokeStyle = 'rgba(255,255,255,0.12)'; ctx.lineWidth = 9;
      ctx.beginPath(); ctx.arc(cx, cy, R, a0, a1); ctx.stroke();
      const grad = ctx.createLinearGradient(cx - R, cy, cx + R, cy);
      grad.addColorStop(0, '#4CC9F0'); grad.addColorStop(0.65, '#FFD166'); grad.addColorStop(1, '#FF5C7A');
      ctx.strokeStyle = grad;
      ctx.beginPath(); ctx.arc(cx, cy, R, a0, a0 + (a1 - a0) * pct); ctx.stroke();
      // 눈금
      ctx.strokeStyle = 'rgba(255,255,255,0.35)'; ctx.lineWidth = 2;
      for (let k = 0; k <= 8; k++) {
        const a = a0 + (a1 - a0) * k / 8, r1 = R - 9, r2 = R - (k % 2 ? 13 : 17);
        ctx.beginPath(); ctx.moveTo(cx + Math.cos(a) * r1, cy + Math.sin(a) * r1); ctx.lineTo(cx + Math.cos(a) * r2, cy + Math.sin(a) * r2); ctx.stroke();
      }
      // 바늘
      const na = a0 + (a1 - a0) * pct;
      ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + Math.cos(na) * (R - 12), cy + Math.sin(na) * (R - 12)); ctx.stroke();
      ctx.fillStyle = '#FFFFFF'; ctx.beginPath(); ctx.arc(cx, cy, 4, 0, Math.PI * 2); ctx.fill();
      // 숫자
      ctx.fillStyle = '#FFFFFF'; ctx.font = '800 ' + Math.round(R * 0.42) + 'px Pretendard, sans-serif';
      ctx.textAlign = 'center'; ctx.textBaseline = 'alphabetic';
      ctx.fillText(String(Math.round(this.speed * 34)), cx, cy - R * 0.22);
      ctx.font = '700 10px Pretendard, sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.55)';
      ctx.fillText('km/h', cx, cy - R * 0.22 + 13);
      if (now < this.boostUntil) { ctx.fillStyle = '#4CC9F0'; ctx.font = '800 11px Pretendard, sans-serif'; ctx.fillText('BOOST', cx, cy - R * 0.62); }
      ctx.textAlign = 'left';
      ctx.restore();
    }

    // 누구에게 당했는지 — 가운데 큰 배너
    if (this.hitBanner && now < this.hitBanner.until) {
      const hb = this.hitBanner, life = (hb.until - now) / 2200;
      const k = life > 0.88 ? (1 - (life - 0.88) / 0.12) : (life < 0.15 ? life / 0.15 : 1);
      ctx.save();
      ctx.globalAlpha = k;
      ctx.translate(W/2, H*0.4);
      ctx.scale(0.9 + k * 0.1, 0.9 + k * 0.1);
      ctx.font = '800 15px Pretendard, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      const line = hb.who + hb.what;
      const bw = Math.min(W * 0.9, Math.max(220, ctx.measureText(line).width + 96)), bh = 78;
      ctx.fillStyle = 'rgba(8,10,16,0.88)';
      if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(-bw/2, -bh/2, bw, bh, 20); ctx.fill(); }
      else ctx.fillRect(-bw/2, -bh/2, bw, bh);
      ctx.strokeStyle = hb.color; ctx.lineWidth = 2;
      if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(-bw/2, -bh/2, bw, bh, 20); ctx.stroke(); }
      ctx.font = '800 30px sans-serif'; ctx.fillStyle = '#fff'; ctx.fillText(hb.icon, -bw/2 + 34, 0);
      ctx.font = '800 20px Pretendard, sans-serif'; ctx.fillStyle = hb.color; ctx.fillText(hb.who, 18, -13);
      ctx.font = '600 13px Pretendard, sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.85)'; ctx.fillText(hb.what, 18, 14);
      ctx.restore();
    }
